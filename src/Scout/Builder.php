...
class Builder extends \Laravel\Scout\Builder
{
     ...
     *
     * @param int $perPage
     * @param  bool|array filter columns form _source
     ...
     *
     * @param string          $boolOccur
     * @param Collection|null $bool
     *
     * @return $this
     */
    public function setBool($boolOccur = 'must', Collection $bool = null)
    {
        null === $bool && $bool = new Collection();
        //create [bool][must]
    ...
    {
        return $this->where($column, 'range', [
            'gte' => $values[0],
            'lte' => $values[1],
        ]);
    }
    ...
    protected function parseWhere($column, $operator, $value, $options = [])
    {
        ...
        } elseif (is_array($column) && is_null($value) && is_null($operator)) { //array, append data
