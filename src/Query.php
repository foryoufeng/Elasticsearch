...
use Foryoufeng\Elasticsearch\Classes\Bulk;
use Foryoufeng\Elasticsearch\Classes\Search;
use Foryoufeng\Elasticsearch\Collection;


/**
 * Class Query
 * @package Foryoufeng\Elasticsearch\Query
 */
class Query
{
     ...
     * @var \Foryoufeng\Elasticsearch\Model
     */
    public $model;
    ...
    protected function getAll($result = [])
    {
        ...
        foreach ($result['hits']['hits'] as $row) {
            ...
            if(isset($row['highlight'])){
                foreach ($row['highlight'] as $k=>$highlight){
                    $attr=$k.'_highlight';
                    $model->$attr= $highlight[0];
                }
            }
        ...
        $new->scroll_id = $result['_scroll_id']??null;
    ...
    protected function getFirst($result = [])
    {
        ...
        if (count($data)) {
            ...
            if(isset($data[0]['highlight'])){
                foreach ($data[0]['highlight'] as $k=>$highlight){
                    $attr=$k.'_highlight';
                    $model->$attr= $highlight[0];
                }
            }
